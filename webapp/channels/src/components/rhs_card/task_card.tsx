...
import {getCurrentUserId, getProfilesInCurrentChannel, makeGetDisplayName} from 'mattermost-redux/selectors/entities/users';
...
export default function TaskCard() {
    ...
    const assigneeValue = useMemo(
        () => assignees.map((id: string) => userMap[id]),
        [userMap, assignees]
    );
    const managerValue = useMemo(
        () => managers.map((id: string) => userMap[id]),
        [userMap, managers]
    );
                    ...
                    {(checklist.items || []).map((item: any, itemIdx: number) => {
                        ...
                        return (
                            <ListGroupItem
                                ...
                                onClick={closed ? undefined : () => updateChecklistItem(idx, itemIdx, {state: 'closed'})}
                            >
