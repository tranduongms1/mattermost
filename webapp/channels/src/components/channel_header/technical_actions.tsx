...
const TechnicalActions = ({channel}: Props) => {
    ...
    const {issuesCount, troublesCount, plansCount, recurringTasksCount, doneTroublesCount, doneIssuesCount} = channelStats;
    const doneCount = doneTroublesCount + doneIssuesCount;
    ...
    const isDoneTasks = rhsState === RHSStates.CHANNEL_DONE_TASKS || (selectedChannelId !== '' && ['done', 'completed'].includes(statuses[0]));
