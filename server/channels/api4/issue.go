...
func (api *API) InitIssue() {
	api.BaseRoutes.Issues.Handle("", api.APISessionRequired(createIssue)).Methods(http.MethodPost)
	api.BaseRoutes.IssuesForChannel.Handle("", api.APISessionRequired(getIssuesForChannel)).Methods(http.MethodGet)
}

func createIssue(c *Context, w http.ResponseWriter, r *http.Request) {
	var post model.Post
	if jsonErr := json.NewDecoder(r.Body).Decode(&post); jsonErr != nil {
		c.SetInvalidParamWithErr("issue", jsonErr)
		return
	}
	...
	post.Type = "custom_issue"
	post.AddProp("creator_id", user.Id)
	post.AddProp("creator_name", user.GetFullName())
	post.AddProp("status", "new")

	auditRec := c.MakeAuditRecord("createIssue", audit.Fail)
	defer c.LogAuditRecWithLevel(auditRec, app.LevelContent)
	audit.AddEventParameterAuditable(auditRec, "issue", &post)
	...
	auditRec.AddEventObjectType("issue")
...
func getIssuesForChannel(c *Context, w http.ResponseWriter, r *http.Request) {
	c.RequireChannelId()
	if c.Err != nil {
		return
	}
