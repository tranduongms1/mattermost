...
func (api *API) InitPlan() {
	...
	api.BaseRoutes.PlanChecklist.Handle("", api.APISessionRequired(updatePlanChecklist)).Methods(http.MethodPatch)
	api.BaseRoutes.PlansForChannel.Handle("", api.APISessionRequired(getPlansForChannel)).Methods(http.MethodGet)
}
...
func getPlansForChannel(c *Context, w http.ResponseWriter, r *http.Request) {
	c.RequireChannelId()
	if c.Err != nil {
		return
	}
...
func updatePlanChecklist(c *Context, w http.ResponseWriter, r *http.Request) {
	c.RequirePostId()
	if c.Err != nil {
		return
	}
	...
	if err := json.NewDecoder(r.Body).Decode(&patch); err != nil || patch == nil {
		c.SetInvalidParamWithErr("planPatch", err)
		return
	}

	plan, appErr := c.App.GetSinglePost(c.AppContext, c.Params.PostId, false)
	if appErr != nil {
		c.Err = appErr
		return
	}

	checklists := plan.GetProp("checklists").([]any)
	...
	plan.AddProp("checklists", checklists)

	rpost, appErr := c.App.UpdatePost(c.AppContext, plan, false)
	if appErr != nil {
		c.Err = appErr
		return
	}
